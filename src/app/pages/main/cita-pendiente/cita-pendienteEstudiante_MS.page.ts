...
export class CitaPendientePage implements OnInit {
  ...
  ionViewWillEnter() {this.getCitas();}
  user(): user_ETD {return this.utilsSvc.getFromLocalStorage('user');}

  async doRefresh(event: any) { await this.getCitas();event.target.complete();
  }
  getCitasHoy(): Cita[] {
    const hoy = new Date();
